...
__export(main_exports, {
  default: () => GMVaultExporterPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian4 = require("obsidian");

// src/PluginController.js
...
var ServerManager = class {
  ...
  _extractParams(method, pathname, route) {
    ...
    for (const match3 of paramMatches) {
      paramNames.push(match3[1]);
    }
    const regex = this._routeToRegex(route);
    const match2 = pathname.match(regex);
    if (match2) {
      for (let i = 0; i < paramNames.length; i++) {
        params[paramNames[i]] = match2[i + 1];
      }
    }
    return params;
  }
...
// src/server/TunnelManager.js
var import_child_process = require("child_process");
var import_os = require("os");
var import_fs = require("fs");
var import_path = require("path");
var import_https = __toESM(require("https"));
var CLOUDFLARED_RELEASES_BASE = "https://github.com/cloudflare/cloudflared/releases/latest/download";
var TunnelManager = class {
  ...
  _getBinDir() {
    if (!this.pluginDir)
      return null;
    return (0, import_path.join)(this.pluginDir, "bin");
  }
  /**
  ...
  _getLocalBinaryPath() {
    ...
    return (0, import_path.join)(binDir, binaryName);
  }
  /**
  ...
  _getDownloadInfo() {
    const os = (0, import_os.platform)();
    const cpuArch = (0, import_os.arch)();
    let filename;
    ...
    return new Promise((resolve, reject) => {
      const handleResponse = (response) => {
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          import_https.default.get(response.headers.location, handleResponse).on("error", reject);
          return;
        }
        ...
        const file = (0, import_fs.createWriteStream)(destPath);
        ...
        file.on("error", (err) => {
          (0, import_fs.unlinkSync)(destPath);
          reject(err);
        });
      };
      import_https.default.get(url, handleResponse).on("error", reject);
    });
  }
  /**
  ...
  async _extractTgz(tgzPath, destDir) {
    ...
    return new Promise((resolve, reject) => {
      try {
        (0, import_child_process.execSync)(`tar -xzf "${tgzPath}" -C "${destDir}"`, {
          encoding: "utf8",
  ...
  async _downloadCloudflared() {
    ...
    if (!downloadInfo) {
      throw new Error(`Unsupported platform: ${(0, import_os.platform)()} ${(0, import_os.arch)()}`);
    }
    const binDir = this._getBinDir();
    if (!binDir) {
    ...
    if (downloadInfo.needsExtraction) {
      const tgzPath = (0, import_path.join)(binDir, "cloudflared.tgz");
      await this._downloadFile(downloadInfo.url, tgzPath);
      await this._extractTgz(tgzPath, binDir);
      try {
        (0, import_fs.unlinkSync)(tgzPath);
      } catch (e) {
      }
    } else {
      await this._downloadFile(downloadInfo.url, localBinaryPath);
    }
    if (os !== "win32" && (0, import_fs.existsSync)(localBinaryPath)) {
      (0, import_fs.chmodSync)(localBinaryPath, 493);
    }
    try {
      (0, import_child_process.execSync)(`"${localBinaryPath}" --version`, {
        encoding: "utf8",
        stdio: ["ignore", "pipe", "pipe"]
      });
  ...
  async _ensureCloudflared(useBundledOnly = false) {
    const os = (0, import_os.platform)();
    const localBinaryPath = this._getLocalBinaryPath();
    if (localBinaryPath && (0, import_fs.existsSync)(localBinaryPath)) {
      try {
        (0, import_child_process.execSync)(`"${localBinaryPath}" --version`, {
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"]
    ...
    try {
      if (os === "win32") {
        const result = (0, import_child_process.execSync)("where cloudflared.exe", {
          encoding: "utf8",
        ...
        const result = (0, import_child_process.execSync)("which cloudflared", {
          encoding: "utf8",
          stdio: ["ignore", "pipe", "ignore"]
        }).trim();
        if (result) {
  ...
  async start(options = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.tunnel = (0, import_child_process.spawn)(command, [
        "tunnel",
        "--url",
      ...
      let timeoutId;
      const tryResolveUrl = (text2) => {
        if (urlResolved)
          return;
        const urlMatch = text2.match(/https:\/\/[a-z0-9-]+\.trycloudflare\.com/gi);
      ...
      this.tunnel.on("close", (code2) => {
        if (urlResolved) {
          return;
        }
        if (code2 !== 0 && code2 !== null && !urlResolved) {
          const fullOutput = output + errorOutput;
          let errorMsg = `cloudflared exited with code ${code2}`;
          if (fullOutput) {
            errorMsg += `
Output: ${fullOutput.substring(0, 500)}`;
      ...
      timeoutId = setTimeout(() => {
        var _a2;
        if (!urlResolved) {
          (_a2 = this.tunnel) == null ? void 0 : _a2.kill();
          this.tunnel = null;
...
var SessionParser = class {
  ...
  _getFileSortOrder() {
    var _a2, _b, _c;
    const sortOrder = ((_b = (_a2 = this.app.vault).getConfig) == null ? void 0 : _b.call(_a2, "fileSortOrder")) || ((_c = this.app.vault.config) == null ? void 0 : _c.fileSortOrder) || "alphabetical";
    return sortOrder;
  }
  /**
  ...
  _sortByObsidianConfig(items) {
    ...
    switch (sortOrder) {
      ...
      case "byModifiedTimeReverse":
        sorted.sort((a, b) => {
          var _a2, _b;
          return (((_a2 = a.stat) == null ? void 0 : _a2.mtime) || 0) - (((_b = b.stat) == null ? void 0 : _b.mtime) || 0);
        });
        break;
      case "byCreatedTime":
        sorted.sort((a, b) => {
          var _a2, _b;
          return (((_a2 = b.stat) == null ? void 0 : _a2.ctime) || 0) - (((_b = a.stat) == null ? void 0 : _b.ctime) || 0);
        });
        break;
      case "byCreatedTimeReverse":
        sorted.sort((a, b) => {
          var _a2, _b;
          return (((_a2 = a.stat) == null ? void 0 : _a2.ctime) || 0) - (((_b = b.stat) == null ? void 0 : _b.ctime) || 0);
        });
        break;
      default:
        sorted.sort((a, b) => a.name.localeCompare(b.name));
  ...
  async parseSession(sessionFolder) {
    ...
    let rootCategoryName = sessionFile ? await this._getRootCategoryName(sessionFile) : sessionFolder.name;
    ...
    const isEmptyName = !rootCategoryName || typeof rootCategoryName === "string" && rootCategoryName.trim() === "";
  ...
  async _getRootCategoryName(sessionFile) {
    ...
    for (const line of lines) {
      const match2 = line.match(h1Regex);
      if (match2) {
  ...
  async _scanFolder(folder, category, sessionFile) {
    ...
    for (const subFolder of sortedFolders) {
      ...
      const hasOnlyImages = imageFiles.length > 0 && subFolder.children.filter((c) => c instanceof import_obsidian.TFile && c.extension === "md").length === 0 && subFolder.children.filter((c) => c.children !== void 0).length === 0;
      if (hasOnlyImages) {
...
// src/renderers/GMVaultJSONBuilder.js
var GMVaultJSONBuilder = class {
  /**
  ...
  buildJSON(session) {
    return {
      categories: session.categories.map(
        (category) => this._buildCategoryJSON(category)
      )
    };
  }
  /**
  ...
  buildLegacyJSON(session) {
    return {
      categories: session.categories.map(
        (category) => this._buildLegacyCategoryJSON(category)
      )
    };
  }
  /**
  ...
  _buildLegacyCategoryJSON(category) {
    const json = {
      ...
      categories: category.categories.map(
        (subCategory) => this._buildLegacyCategoryJSON(subCategory)
      )
    };
...
<head>
	...
	<style>
		...
		* {
		...
		/* T\xEDtulo de p\xE1gina de Notion (despu\xE9s del cover) */
		.notion-page-title {
		...
		/* Evitar l\xEDneas vac\xEDas extra de <br> */
		.notion-content br {
			display: block;
			content: "";
			margin-top: 0;
		...
		/* Evitar doble espaciado con p\xE1rrafos que solo contienen br */
...
<body>
	...
	<script>
		...
		console.log('\u{1F680} Script cargado - VERSION 2026-01-29-v7');
		
		// Estado del usuario
		var userRole = { isGM: false, isPlayer: true, isCoGM: false };
		
		// Detectar si estamos en un modal desde el par\xE1metro de URL (evita parpadeo)
		var isInModal = new URLSearchParams(window.location.search).get('inModal') === 'true';
		if (isInModal) {
			console.log('\u{1F512} Detectado inModal=true en URL - aplicando estilos inmediatamente');
		...
		// Funci\xF3n para convertir mentions a plain
		function convertMentionsToPlain() {
			console.log('\u{1F464} Convirtiendo mentions a plain para Player');
			const linkMentions = document.querySelectorAll('.notion-mention--link');
			console.log('\u{1F50D} Mentions --link encontrados:', linkMentions.length);
		...
		// Funci\xF3n para a\xF1adir botones de share y click handlers a las im\xE1genes
		function addShareButtonsToImages() {
			console.log('\u{1F5BC}\uFE0F A\xF1adiendo botones de share y click handlers a im\xE1genes');
			const images = document.querySelectorAll('.notion-content img');
			console.log('\u{1F50D} Im\xE1genes encontradas:', images.length);
			...
			images.forEach(function(img) {
				...
				// A\xF1adir clase clickable y cursor pointer
				...
				if (!img.dataset.clickListenerAdded) {
					...
					img.addEventListener('click', function(e) {
						...
						console.log('\u{1F50D} Abriendo imagen en modal:', imageUrl);
				...
				shareBtn.addEventListener('click', function(e) {
					...
					console.log('\u{1F5BC}\uFE0F Compartiendo imagen:', imageUrl);
			...
			console.log('\u2705 Botones de share y click handlers a\xF1adidos');
		}
		
		// Funci\xF3n para eliminar botones de share (para Players)
		function removeShareButtons() {
			const buttons = document.querySelectorAll('.notion-image-share-button');
			buttons.forEach(function(btn) {
				btn.remove();
			});
			console.log('\u{1F5D1}\uFE0F Botones de share eliminados para Player');
		}
		
		// Funci\xF3n para deshabilitar mentions (dentro de modal - sin navegaci\xF3n)
		function disableMentionsInModal() {
			console.log('\u{1F6AB} Deshabilitando mentions dentro del modal');
			const linkMentions = document.querySelectorAll('.notion-mention--link');
			console.log('\u{1F50D} Mentions --link encontrados:', linkMentions.length);
			linkMentions.forEach(function(mention) {
				mention.classList.remove('notion-mention--link');
		...
		window.addEventListener('message', function(event) {
			if (event.data && event.data.type === 'setUserRole') {
				console.log('\u{1F4E8} Rol de usuario recibido:', event.data);
				userRole.isGM = event.data.isGM;
				userRole.isPlayer = event.data.isPlayer;
				userRole.isCoGM = event.data.isCoGM;
				
				// Si est\xE1 dentro de un modal, deshabilitar navegaci\xF3n de mentions
				if (event.data.isInModal) {
					console.log('\u{1F512} Contenido en modal - deshabilitando navegaci\xF3n');
					disableMentionsInModal();
				} else if (event.data.isPlayer) {
					convertMentionsToPlain();
				}
				
				// Todos pueden compartir im\xE1genes (GM, coGM y Players)
				console.log('\u{1F5BC}\uFE0F A\xF1adiendo botones de share a im\xE1genes para todos');
		...
		if (window.parent && window.parent !== window) {
			console.log('\u{1F4E4} Preguntando al padre si somos Player...');
			try {
		...
		console.log('\u2705 Listener de mensajes a\xF1adido');
		
		// Manejar clics en mentions - usar el sistema de modales de GM Vault
		// Si GM Vault no encuentra la p\xE1gina por ID, intentar buscarla por URL
		(function() {
			const mentions = document.querySelectorAll('.notion-mention--link');
			console.log('\u{1F50D} Mentions encontrados:', mentions.length);
			...
			mentions.forEach(function(mention) {
				...
				console.log('\u{1F4CC} Mention configurado:', { pageId, pageName, pageUrl });
				...
				mention.addEventListener('click', function(e) {
					...
					console.log('\u{1F5B1}\uFE0F Click en mention:', { pageId, pageName, pageUrl });
					...
					try {
						if (window.parent && window.parent !== window) {
							console.log('\u{1F4E8} Enviando mensaje a window.parent...');
							window.parent.postMessage({
					...
					// Fallback: navegar directamente a la p\xE1gina
					console.log('\u{1F517} Navegando directamente a:', pageUrl);
  ...
  _addNotionClasses(html) {
    ...
    const addClass = (tag, className) => {
      ...
      return processed.replace(regex, (match2, attrs) => {
        if (attrs && attrs.trim()) {
    ...
    processed = processed.replace(/(<ul[^>]*class="[^"]*notion-bulleted-list[^>]*>)([\s\S]*?)(<\/ul>)/g, (match2, open, content, close) => {
      const processedContent = content.replace(/<li([^>]*)>/gi, (liMatch, liAttrs) => {
        if (!liAttrs || !liAttrs.includes('class="notion-bulleted-list-item"')) {
          return liAttrs.trim() ? `<li${liAttrs} class="notion-bulleted-list-item">` : `<li class="notion-bulleted-list-item">`;
        }
        return liMatch;
      });
      return open + processedContent + close;
    });
    processed = processed.replace(/(<ol[^>]*class="[^"]*notion-numbered-list[^>]*>)([\s\S]*?)(<\/ol>)/g, (match2, open, content, close) => {
      const processedContent = content.replace(/<li([^>]*)>/gi, (liMatch, liAttrs) => {
        if (!liAttrs || !liAttrs.includes('class="notion-numbered-list-item"')) {
          return liAttrs.trim() ? `<li${liAttrs} class="notion-numbered-list-item">` : `<li class="notion-numbered-list-item">`;
        }
        return liMatch;
      });
      return open + processedContent + close;
    ...
    processed = processed.replace(/<code(?![^>]*class="[^"]*notion-text-code)(?![^>]*<pre)([^>]*)>/gi, (match2, attrs) => {
      return attrs.trim() ? `<code${attrs} class="notion-text-code">` : `<code class="notion-text-code">`;
    });
    processed = addClass("a", "notion-text-link");
    processed = processed.replace(/<(strong)(?![^>]*class="[^"]*notion-text-bold)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-bold">` : `<${tag} class="notion-text-bold">`;
    });
    processed = processed.replace(/<(b)(?![^>]*class="[^"]*notion-text-bold)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-bold">` : `<${tag} class="notion-text-bold">`;
    });
    processed = processed.replace(/<(em)(?!bed\b)(?![^>]*class="[^"]*notion-text-italic)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-italic">` : `<${tag} class="notion-text-italic">`;
    });
    processed = processed.replace(/<(i)(?!mg\b)(?![^>]*class="[^"]*notion-text-italic)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-italic">` : `<${tag} class="notion-text-italic">`;
    });
    processed = addClass("u", "notion-text-underline");
    processed = processed.replace(/<(s)(?![^>]*class="[^"]*notion-text-strikethrough)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-strikethrough">` : `<${tag} class="notion-text-strikethrough">`;
    });
    processed = processed.replace(/<(del)(?![^>]*class="[^"]*notion-text-strikethrough)([^>]*)>/gi, (match2, tag, attrs) => {
      return attrs.trim() ? `<${tag}${attrs} class="notion-text-strikethrough">` : `<${tag} class="notion-text-strikethrough">`;
    });
  ...
  _convertWikiLinksInMarkdown(markdown, baseUrl = null) {
    ...
    const codeRegex = /```[\s\S]*?```|`[^`\n]+`/g;
    let match2;
    while ((match2 = codeRegex.exec(markdown)) !== null) {
      if (match2.index > lastIndex) {
        const beforeCode = markdown.substring(lastIndex, match2.index);
        parts.push({ text: beforeCode, isCode: false });
      }
      parts.push({ text: match2[0], isCode: true });
    ...
    const processedParts = parts.map((part) => {
      ...
      return part.text.replace(wikiLinkRegex, (match3, linkContent) => {
        const parts2 = linkContent.split("|");
        const linkPath = parts2[0].trim();
        const displayName = (parts2[1] || parts2[0]).trim();
        if (this.pageMap) {
          const pageInfo = this.pageMap.get(linkPath.toLowerCase());
          if (pageInfo) {
            const urlBase2 = baseUrl || this.baseUrl;
            const pageUrl = urlBase2 ? `${urlBase2}/pages/${pageInfo.slug}` : `/pages/${pageInfo.slug}`;
        ...
        const urlBase = baseUrl || this.baseUrl;
        const href = urlBase ? `${urlBase}/pages/${slug}` : `/pages/${slug}`;
  ...
  _convertWikiLinksToMentions(html, baseUrl = null) {
    ...
    let match2;
    while ((match2 = existingElementsRegex.exec(html)) !== null) {
      if (match2.index > lastIndex) {
        const beforeElement = html.substring(lastIndex, match2.index);
        parts.push({ text: beforeElement, isElement: false });
      }
      parts.push({ text: match2[0], isElement: true });
      lastIndex = match2.index + match2[0].length;
    }
    ...
    const processedParts = parts.map((part) => {
      ...
      return part.text.replace(wikiLinkRegex, (match3, linkContent) => {
        ...
        if (pageInfo) {
          const urlBase = baseUrl || this.baseUrl;
          const pageUrl = urlBase ? `${urlBase}/pages/${pageInfo.slug}` : `/pages/${pageInfo.slug}`;
          return `<span class="notion-mention notion-mention--link" data-mention-page-id="${pageInfo.id}" data-mention-page-name="${this._escapeHtml(pageInfo.name)}" data-mention-page-url="${pageUrl}" role="button" tabindex="0" aria-label="Open ${this._escapeHtml(pageInfo.name)}">${this._escapeHtml(displayName)}</span>`;
        } else {
          return `<span class="notion-mention notion-mention--plain" data-mention-page-name="${this._escapeHtml(linkPath)}">${this._escapeHtml(displayName)}</span>`;
        }
  ...
  _convertWikiLinksInHTML(html, baseUrl = null) {
    ...
    while ((match2 = linkRegex.exec(html)) !== null) {
      if (match2.index > lastIndex) {
        const beforeLink = html.substring(lastIndex, match2.index);
        parts.push({ text: beforeLink, isLink: false });
      }
      parts.push({ text: match2[0], isLink: true });
      lastIndex = match2.index + match2[0].length;
    }
    if (lastIndex < html.length) {
    ...
    const processedParts = parts.map((part) => {
      ...
      for (const pattern of wikiLinkPatterns) {
        processedText = processedText.replace(pattern, (match3, linkContent) => {
          const parts2 = linkContent.split("|");
          const linkPath = parts2[0].trim();
          const displayName = (parts2[1] || parts2[0]).trim();
          if (this.pageMap) {
  ...
  _convertRelativeUrlsToAbsolute(html, baseUrl) {
    ...
    html = html.replace(/<a\s+([^>]*\s+)?href="(\/[^"]+)"/gi, (match2, attrs, href) => {
      if (href.startsWith("/") && !href.startsWith("//")) {
        return `<a ${attrs || ""}href="${normalizedBase}${href}"`.replace(/\s+/g, " ").trim();
      }
      return match2;
    });
    html = html.replace(/<img\s+([^>]*\s+)?src="(\/[^"]+)"/gi, (match2, attrs, src) => {
...
<body>
	...
	<script>
		...
		console.log('\u{1F5BC}\uFE0F Gallery script cargado');
		...
		function addImageHandlers() {
			...
			images.forEach(function(img) {
				...
				// A\xF1adir bot\xF3n de share si no existe
		...
		// Tambi\xE9n escuchar setUserRole para re-aplicar handlers
		window.addEventListener('message', function(event) {
			if (event.data && event.data.type === 'setUserRole') {
				console.log('\u{1F4E8} Rol recibido en gallery:', event.data);
				addImageHandlers();
			}
		});
  ...
  _convertTagsToNotionTags(html) {
    ...
    let match2;
    while ((match2 = codeBlockRegex.exec(html)) !== null) {
      if (match2.index > lastIndex) {
        const beforeCode = html.substring(lastIndex, match2.index);
        parts.push({ text: beforeCode, isCode: false });
      }
      parts.push({ text: match2[0], isCode: true });
      lastIndex = match2.index + match2[0].length;
  ...
  _addTargetToExternalLinks(html) {
    return html.replace(/<a\s+([^>]*\s+)?href="([^"]+)"([^>]*)>/gi, (match2, attrs1, href, attrs2) => {
...
// src/exporters/VaultExporter.js
...
var VaultExporter = class {
  ...
  async _buildPageMap(folder) {
    for (const child of folder.children || []) {
      if (child instanceof import_obsidian2.TFile && child.extension === "md") {
        ...
        const hasOnlyImages = imageFiles.length > 0 && child.children.filter((c) => c instanceof import_obsidian2.TFile && c.extension === "md").length === 0 && child.children.filter((c) => c.children !== void 0).length === 0;
        if (hasOnlyImages) {
          const pageId = generatePageId();
  ...
  async _exportFolder(folder, sessionFile = null) {
    ...
    for (const subFolder of sortedFolders) {
      const imageFiles = await this._getImageFiles(subFolder);
      const hasOnlyImages = imageFiles.length > 0 && subFolder.children.filter((c) => c instanceof import_obsidian2.TFile && c.extension === "md").length === 0 && subFolder.children.filter((c) => c.children !== void 0).length === 0;
      if (hasOnlyImages) {
  ...
  async _exportPage(file, parentFolder) {
    ...
    const pageId = (pageInfo == null ? void 0 : pageInfo.id) || generatePageId();
        ...
        imagesHtml += `
					<div class="notion-column">
						<div class="notion-image-container" style="padding: 20px; text-align: center; background: #f5f5f5; border-radius: 4px;">
							<p style="color: #666; margin: 0;">\u{1F5BC}\uFE0F ${this._escapeHtml(imageFile.name)}</p>
    ...
    const htmlContent = `
			...
			<div class="notion-callout" style="background: #fef3c7; border-left: 3px solid #f59e0b; padding: 12px; margin: 16px 0; border-radius: 4px;">
				<p style="margin: 0; color: #92400e;">
					<strong>\u{1F4A1} Tip:</strong> Sube las im\xE1genes a un servicio de hosting (Imgur, Cloudinary) y usa URLs externas.
				</p>
			</div>
			${imagesHtml}
		`;
  ...
  _convertWikiLinksToMentions(html) {
    const wikiLinkRegex = /\[\[([^\]]+)\]\]/g;
    return html.replace(wikiLinkRegex, (match2, linkContent) => {
  ...
  async _processImages(html, contextFolder) {
    ...
    for (const match2 of matches) {
      const [fullMatch, imagePath] = match2;
  ...
  _createImagePlaceholder(fileName) {
    return `<div class="notion-image-container" style="padding: 20px; text-align: center; background: #f5f5f5; border-radius: 4px;">
			<p style="color: #666; margin: 0;">\u{1F5BC}\uFE0F ${this._escapeHtml(fileName)}</p>
			<p style="color: #999; font-size: 12px; margin: 5px 0 0 0;">(Usa URL externa)</p>
		</div>`;
  }
  /**
  ...
  _addNotionClasses(html) {
    ...
    processed = processed.replace(/<a\s+href="([^"]+)"([^>]*)>/gi, (match2, href, rest) => {
...
${cleanedContent}`;
  ...
  _escapeHtml(text2) {
    ...
    return String(text2).replace(/[&<>"']/g, (m) => map[m]);
  }
};
...
var PluginController = class {
  ...
  _getPluginDir() {
    ...
    try {
      ...
      const basePath = adapter.basePath || ((_a2 = adapter.getBasePath) == null ? void 0 : _a2.call(adapter));
      if (!basePath) {
  ...
  async enableServer() {
    if (!this.currentSessionFolder) {
      new import_obsidian3.Notice('Please select a session folder first using the "Select session folder" command');
      return;
    }
    try {
      ...
      new import_obsidian3.Notice("\u23F3 Creating public HTTPS tunnel...");
      ...
      new import_obsidian3.Notice(`\u2705 GM Vault access enabled (HTTPS):
${publicUrl}

Use this URL in GM Vault:
${publicUrl}/gm-vault`, 1e4);
      await this._saveSettings();
      await this.copyGmVaultUrl();
    } catch (error) {
      new import_obsidian3.Notice(`\u274C Error starting server: ${error.message}`);
    }
  }
  /**
  ...
  async disableServer() {
    try {
      ...
      await this.serverManager.stop();
      this.publicUrl = null;
      new import_obsidian3.Notice("\u2705 GM Vault access disabled");
      await this._saveSettings();
    } catch (error) {
      new import_obsidian3.Notice(`\u274C Error stopping server: ${error.message}`);
    }
  }
  /**
  ...
  async showPublicUrl() {
    var _a2;
    const url = ((_a2 = this.tunnelManager) == null ? void 0 : _a2.getPublicUrl()) || this.publicUrl;
    if (!url) {
      new import_obsidian3.Notice('\u274C No active tunnel. Run "Enable GM Vault access" first.');
      return;
    }
    new import_obsidian3.Notice(`\u{1F310} Tunnel public HTTPS URL:
${url}

\u2022 JSON for GM Vault: ${url}/gm-vault
\u2022 Pages: ${url}/pages/:slug`, 1e4);
  ...
  async copyGmVaultUrl() {
    ...
    if (navigator.clipboard) {
      try {
        ...
        new import_obsidian3.Notice(`\u2705 GM-vault URL copied to clipboard:
${gmVaultUrl}`);
      } catch (e) {
        new import_obsidian3.Notice(`\u274C Error copying to clipboard: ${e.message}`);
      }
    } else {
      new import_obsidian3.Notice(`\u{1F4CB} URL GM-vault:
${gmVaultUrl}`, 1e4);
    }
  }
  /**
  ...
  async exportVaultToJson() {
    if (!this.currentSessionFolder) {
      new import_obsidian3.Notice("\u274C Please select a session folder first");
      return;
    }
    try {
      new import_obsidian3.Notice("\u23F3 Exporting vault...");
      ...
      new import_obsidian3.Notice(`\u2705 Vault exported successfully!

\u{1F4C1} ${fileName}
\u{1F4CA} ${pageCount} pages
\u{1F4BE} ${sizeKB} KB

\u{1F4A1} Note: Local images are omitted. Use external URLs to include images.

Import this file in GM Vault`, 1e4);
    } catch (error) {
      console.error("Error exporting vault:", error);
      new import_obsidian3.Notice(`\u274C Error exporting: ${error.message}`);
    }
  }
  /**
  ...
  async selectSessionFolder() {
    ...
    class FolderSuggester extends import_obsidian3.SuggestModal {
      constructor(app, folders2) {
        super(app);
      ...
      async onChooseSuggestion(folder, evt) {
        ...
        new import_obsidian3.Notice(`\u2705 Session folder selected: ${folder.path}`);
    ...
    this.serverManager.registerRoute("GET", "/pages/:slug", async (req, res, params) => {
      ...
      try {
        ...
        if (imageFolder) {
          ...
          if (images.length > 0) {
            const baseUrl2 = this.publicUrl || ((_a2 = this.tunnelManager) == null ? void 0 : _a2.getPublicUrl()) || `http://localhost:${this.port}`;
            const html2 = this.markdownRenderer.renderImageGallery(images, imageFolder.name, baseUrl2);
            this.serverManager.sendHTML(res, html2);
            return;
          }
        }
        ...
        const baseUrl = this.publicUrl || ((_b = this.tunnelManager) == null ? void 0 : _b.getPublicUrl()) || `http://localhost:${this.port}`;
      ...
      for (const child of folder.children || []) {
        if (child instanceof import_obsidian3.TFolder) {
          ...
          if (folderSlug === slug || child.name.toLowerCase() === slug) {
            ...
            const hasOnlyImages = imageFiles.length > 0 && child.children.filter((c) => c instanceof import_obsidian3.TFile && c.extension === "md").length === 0 && child.children.filter((c) => c.children !== void 0).length === 0;
            if (hasOnlyImages) {
              return child;
            }
          }
  ...
  _preprocessEmbedImages(markdown, sourcePath, baseUrl) {
    ...
    while ((match2 = codeRegex.exec(markdown)) !== null) {
      if (match2.index > lastIndex) {
        parts.push({ text: markdown.substring(lastIndex, match2.index), isCode: false });
      }
      parts.push({ text: match2[0], isCode: true });
      lastIndex = match2.index + match2[0].length;
    }
    ...
    const processedParts = parts.map((part) => {
      ...
      return part.text.replace(embedRegex, (fullMatch, linkContent) => {
        ...
        if (!dest || !(dest instanceof import_obsidian3.TFile) || !imageExtensions.includes(dest.extension.toLowerCase())) {
          return fullMatch;
        }
        const pathSegments = dest.path.split("/");
        const encodedPath = pathSegments.map((segment) => encodeURIComponent(segment)).join("/");
        const url = `${baseUrl.replace(/\/$/, "")}/images/${encodedPath}`;
        const alt = ((_a2 = linkContent.split("|")[1]) == null ? void 0 : _a2.trim()) || dest.basename;
        return `![${alt}](${url})`;
      });
    });
    return processedParts.join("");
  ...
  async _getImageFilesFromFolder(folder) {
    ...
    const imageExtensions = ["jpg", "jpeg", "png", "gif", "webp", "svg"];
    const imageFiles = [];
    const baseUrl = this.publicUrl || ((_a2 = this.tunnelManager) == null ? void 0 : _a2.getPublicUrl()) || `http://localhost:${this.port}`;
    for (const child of folder.children || []) {
      if (child instanceof import_obsidian3.TFile) {
        const ext = child.extension.toLowerCase();
        if (imageExtensions.includes(ext)) {
  ...
  async _saveSettings() {
    ...
    await this.plugin.saveData({
      port: this.port,
      sessionFolderPath: ((_a2 = this.currentSessionFolder) == null ? void 0 : _a2.path) || null,
      serverEnabled: ((_b = this.serverManager) == null ? void 0 : _b.isRunning()) || false,
      publicUrl: ((_c = this.tunnelManager) == null ? void 0 : _c.getPublicUrl()) || this.publicUrl || null,
      useBundledCloudflared: this.useBundledCloudflared
    });
  }
};
var GMVaultSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("Use bundled cloudflared only").setDesc("Ignore system cloudflared and use only the plugin's copy (downloads automatically if missing). Useful to test the auto-download or to avoid conflicts with the system installation.").addToggle((toggle) => {
      toggle.setValue(this.controller.useBundledCloudflared).onChange(async (value) => {
...
// src/main.js
var GMVaultExporterPlugin = class extends import_obsidian4.Plugin {
  /**
